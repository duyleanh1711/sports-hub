...
const MenuTrigger = ({ className, ref, ...props }: MenuTriggerProps) => (
  <Button
    ...
    className={cx(
      ...
      "*:data-[slot=chevron]:size-5 sm:*:data-[slot=chevron]:size-4 cursor-pointer",
      className,
    )}
    {...props}
  />
...
const menuContentStyles = tv({
  base: "grid max-h-[inherit] grid-cols-[auto_1fr] gap-y-1 overflow-y-auto overflow-x-hidden overscroll-contain p-1 outline-hidden [clip-path:inset(0_0_0_0_round_calc(var(--radius-xl)-(--spacing(1))))] [&>[data-slot=menu-section]+[data-slot=menu-section]:not([class*='mt-']):not([class*='my-'])]:mt-3 m-0",
});
      ...
      {(values) => (
        <>
          ...
          {values.hasSubmenu && (
            <ChevronRightIcon
              data-slot="chevron"
              className="absolute end-2 size-3.5"
            />
          )}
        </>
      )}
